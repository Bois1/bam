...
class Hand {
    ...
    generateFullHand() {
        ...
        return JSON.stringify({ id, index_finger, middle_finger, name, mobile, age, address, voterarea });
    }
...
function clearPrints() {
    ...
    let verifyFingers2 = document.getElementById("middleVerificationFingers");
    ...
    if (verifyFingers2) {
        for (let finger of verifyFingers2.children) {
            finger.innerHTML = `<span class="icon icon-middlefinger-not-enrolled" title="not_enrolled"></span>`;
        }
    }
}
...
function getNextNotEnrolledID() {
    ...
    let verifyFinger2Element = findElementNotEnrolled(verifyFingers2);
    ...
    if (verifyFinger2Element !== null && verifyUserId !== "") {
        return verifyFinger2Element.id;
    }
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            if (this.responseText !== null && this.responseText !== "") {
                let response = this.responseText; // Use this.responseText directly
                console.log(response);
                if (response === "match") { // Compare directly without parsing as JSON
                    showMessage(successMessage, "success");
                    // detailElement.innerHTML = `<div class="col text-center">
                    //             <label for="fullname" class="my-text7 my-pri-color">Fullname</label>
                    //             <input type="text" id="fullname" class="form-control" value="${response[0].name}">
                    //         </div>
                    //         <div class="col text-center">
                    //             <label for="email" class="my-text7 my-pri-color">Email</label>
                    //             <input type="text" id="email" class="form-control" value="${response[0].id}">
                    //         </div>`;
