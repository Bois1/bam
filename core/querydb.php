...
function setUserFmds(
    ...
    $param_type = "isssssss"; // Assuming id is an integer and the rest are strings
...
function getUserFmds($user_id){
    $myDatabase = new Database();
    $sql_query = "select indexfinger, middlefinger, name from voters where id=?";
    $param_type = "i";
    $param_array = [$user_id];
...
function deleteVoter($user_id){
    $myDatabase = new Database();
    $sql_query = "delete from voters where id=?";
    $param_type = "i";
    $param_array = [$user_id];
    $delete_info = $myDatabase->delete($sql_query, $param_type, $param_array);
    return json_decode($delete_info);
}
